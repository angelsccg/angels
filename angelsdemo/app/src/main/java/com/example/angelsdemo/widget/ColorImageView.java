...
public class ColorImageView extends ImageView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// L.e("hasBorderColor = " + hasBorderColor + " , mBorderColor = " +
		// mBorderColor);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// 以宽度为标准，等比例缩放view的高度
		setMeasuredDimension(viewWidth, getDrawable().getIntrinsicHeight()
				* viewWidth / getDrawable().getIntrinsicWidth());
		// Log.e("view's width = " + getMeasuredWidth() + " , view's height = "
		// + getMeasuredHeight());

		// 根据drawable，去得到一个和view一样大小的bitmap
		BitmapDrawable drawable = (BitmapDrawable) getDrawable();
		Bitmap bm = drawable.getBitmap();
		mBitmap = Bitmap.createScaledBitmap(bm, getMeasuredWidth(),
