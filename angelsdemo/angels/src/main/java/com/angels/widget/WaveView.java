...
/**
 * Created by chencg on 2018/3/22.
 */
...
public class WaveView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawPath(getPath(offset),mPaint);
        canvas.drawPath(getPath(offset + waveWidth/4),mPaint);
        canvas.drawPath(getPath(offset + waveWidth/2),mPaint);
    }
    //初始化paint，没什么可说的。
    private void initView(){
        mPaint = new Paint();
//        mPaint.setColor(Color.RED);
        mPaint.setColor(Color.argb(100,227,249,255));
    ...
    private Path getPath(float offset){
        ...
        Path mPath = new Path();
        mPath.moveTo(-itemWidth * 3 + offset, baseLine );//起始坐标
        //核心的代码就是这里
        for (int i = -3; i < 2; i++) {
