...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	protected void sendResponseMessage(HttpResponse response)
	{
		...
		try
		{
			...
			ACLog.i("sendResponseMessage-->totalSize: " + totalSize);

			if (file.exists() && totalSize == file.length())
			{
				ACLog.i("sendResponseMessage-->Output file already exists. Skipping download.(输出文件已经存在。不下载。)");
			} else if (tempFile.exists())
			{
				// response.addHeader("Range", "bytes=" + tempFile.length()
				// +
				// "-");
				ACLog.i("yahooo: " + response.getEntity().getContentLength());
				previousFileSize = tempFile.length();

				ACLog.i( "sendResponseMessage-->File is not complete, download now.(文件不完整,现在下载。)");
				ACLog.i("File length:" + tempFile.length() + " totalSize:" + totalSize);
			...
			ACLog.i("sendResponseMessage-->Download completed successfully.(下载成功完成)");
			result = bytesCopied;
