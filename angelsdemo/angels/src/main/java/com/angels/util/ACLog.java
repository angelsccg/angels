...
public class ACLog {
    private static final String TAG = "Angels";
...
//    public static char m_cFileSaveLogType = 0;
    ...
    private static void SaveLogFile(String strMsg)
    {
        ...
        do // 非循环，只是为了减少分支缩进深度
        {
            ...
            // 没有指定日志文件存放位置的话，就写到默认位置，即 SD 卡根目录下的 angels_log 目录中
            if (m_strLogFolderPath.trim().equals( "" ) )
            {
                String strSaveLogPath = Environment.getExternalStorageDirectory() + "/angels/angels_log";
            ...
            if ( true != fileLogFilePath.exists() )
            {
                try 
                {
                    ...
                    android.util.Log.d( TAG, "Create log file success!" );
                } 
                catch (IOException e) 
                {
                    e.printStackTrace();
    ...
    public static void d(  String tag,String msg,boolean isSaveLog )
    {
        // 空字符串或全是空格的字符串做为日志输出没有意义！
        if ( ( null == msg ) || 
             ( true == msg.trim().equals( "" ) ) )
