package com.angels.manager.bluetooth;
...
public class BluetoothLeService {
    ...
    private BluetoothLeService() {
        ...
        if (!value) ACLogUtil.e("蓝牙适配器adapter初始化失败!!!");
    }
    ...
    public boolean connect(final String address) {
        if (mBluetoothAdapter == null || address == null) {
            ACLogUtil.e("蓝牙适配器没有初始化获取mac地址未指明。");
            return false;
        }
        final BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(address);
        if (device == null) {
            ACLogUtil.e("蓝牙设备没有发现，无法连接。");
            return false;
        }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                ACLogUtil.e("蓝牙通信服务回调失败：status=" + status);
            }
        }
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            ACLogUtil.e("onDescriptorWrite: status=" + status + "   uuid=" + descriptor.getUuid().toString());
        }
