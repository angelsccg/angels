...
//			.fromString(SampleGattAttributes.HEART_RATE_MEASUREMENT);
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status,
				int newState) {
			ACLogUtil.i(  "蓝牙-->onConnectionStateChange-->"+ status + "--" + newState);
			String intentAction;
...
//			}
	...
	public boolean connect(final String address) {
		if (mBluetoothAdapter == null || address == null) {
			Log.i(TAG,
					"蓝牙-->BluetoothAdapter not initialized or unspecified address.");
			return false;
		}
		...
		if (device == null) {
			Log.i(TAG, "蓝牙-->Device not found.  Unable to connect.");
			return false;
		}
...
//		}else{
		ACLogUtil.i(  "蓝牙-->开始连接connectGatt");
		mBluetoothGatt = device.connectGatt(this, false, mGattCallback);
//		}
		
		if (mBluetoothGatt.connect()) { // 连接远程设备。
			mConnectionState = STATE_CONNECTING;// 
			Log.i(TAG, "蓝牙-->Trying to create a new connection.");
